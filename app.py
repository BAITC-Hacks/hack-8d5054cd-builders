...
def _rating_panel(card: dict[str, Any], *, compact: bool = False, preview: bool = False) -> dict[str, Any]:
    ...
    with st.expander("Из чего складывается рейтинг", expanded=False):
        ...
        st.caption("Это рейтинг полноты, не экспертиза решения. Пустые поля, заглушки и явный мусор дают 0; короткий ответ — половину веса. Для данных и критериев дополнительно проверяются явное отсутствие сведений и общие пожелания.")
